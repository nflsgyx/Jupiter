...
public class TicketMasterClient {
	...
	public List<Item> search(double lat, double lon, String keyword) {
		...
		try {
			...
			// object referred to by the URL. The HttpUrlConnection class allows us to perform basic HTTP requests without the use of any additional libraries. 
			HttpURLConnection connection = (HttpURLConnection) new URL(url).openConnection(); 
			connection.setRequestMethod("GET");
			// Get the status code from an HTTP response message. To execute the request we // can use the getResponseCode(), connect(), getInputStream() or
...
//			// We can use the getResponseCode(), connect(), getInputStream() or getOutputStream() methods.
//			int responseCode = connection.getResponseCode(); 
//			System.out.println("Sending requets to url: " + url); 
//			System.out.println("Response code: " + responseCode);
//			
			...
			if (!embedded.isNull("venues")) {
				...
				for (int i = 0; i < venues.length(); ++i) {
					...
					if (!venue.isNull("address")) {
						...
						if (!address.isNull("line2")) { builder.append(",");
							builder.append(address.getString("line2"));
						}
						if (!address.isNull("line3")) { builder.append(",");
